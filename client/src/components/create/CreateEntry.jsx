...
import { Url } from "../../assets/data.js";
const CreateEntry = ({ setOpen }) => {
  const { user } = useContext(AuthContext);
  const [info, setInfo] = useState({});
  const { data } = useFetch(`${Url}/entries/fetchMealsAndRoutines/${user._id}`);
  ...
  const handleClick = async (e) => {
    ...
    try {
      await axios.post(`${Url}/entries/`, newEntry, {
      ...
      <div className="mContainer">
        ...
        <form>
          ...
          <div className="formInput" id="options">
            ...
            <select id="meals" multiple onChange={handleMultiSelectChange}>
          ...
          <div className="formInput" id="options">
            ...
            <select id="routines" multiple onChange={handleMultiSelectChange}>
              {data?.routines?.map((routine, index) => (
                <option key={index} value={routine._id}>
                  {routine.name}
                </option>
              ))}
            </select>
